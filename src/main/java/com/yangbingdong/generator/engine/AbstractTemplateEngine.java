...
import com.yangbingdong.generator.InjectionConfig;
...
import com.yangbingdong.generator.config.rules.FileType;
...
public abstract class AbstractTemplateEngine {
        ...
        try {
            ...
            for (TableInfo tableInfo : tableInfoList) {
                ...
                if (null != injectionConfig) {
                    ...
                    if (CollectionUtils.isNotEmpty(focList)) {
                        for (FileOutConfig foc : focList) {
                            writer(objectMap, foc.getTemplatePath(), foc.outputFile(tableInfo), isCreate(FileType.OTHER, foc.outputFile(tableInfo)));
                        }
                    }
                }
                // Mp.java
                ...
                if (null != entityName && null != pathInfo.get(ConstVal.ENTITY_PATH)) {
                    ...
                    writer(objectMap, templateFilePath(template.getEntity(getConfigBuilder().getGlobalConfig().isKotlin())), entityFile, isCreate(FileType.ENTITY, entityFile));
                }
                // MpMapper.java
                if (null != tableInfo.getMapperName() && null != pathInfo.get(ConstVal.MAPPER_PATH)) {
                    String mapperFile = String.format((pathInfo.get(ConstVal.MAPPER_PATH) + File.separator + tableInfo.getMapperName() + suffixJavaOrKt()), entityName);
                    writer(objectMap, templateFilePath(template.getMapper()), mapperFile, isCreate(FileType.MAPPER, mapperFile));
                }
                // MpMapper.xml
                if (null != tableInfo.getXmlName() && null != pathInfo.get(ConstVal.XML_PATH)) {
                    String xmlFile = String.format((pathInfo.get(ConstVal.XML_PATH) + File.separator + tableInfo.getXmlName() + ConstVal.XML_SUFFIX), entityName);
                    writer(objectMap, templateFilePath(template.getXml()), xmlFile, isCreate(FileType.XML, xmlFile));
                }
                // IMpService.java
                if (null != tableInfo.getServiceName() && null != pathInfo.get(ConstVal.SERVICE_PATH)) {
                    String serviceFile = String.format((pathInfo.get(ConstVal.SERVICE_PATH) + File.separator + tableInfo.getServiceName() + suffixJavaOrKt()), entityName);
                    writer(objectMap, templateFilePath(template.getService()), serviceFile, isCreate(FileType.SERVICE, serviceFile));
                }
                // MpServiceImpl.java
                if (null != tableInfo.getServiceImplName() && null != pathInfo.get(ConstVal.SERVICE_IMPL_PATH)) {
                    String implFile = String.format((pathInfo.get(ConstVal.SERVICE_IMPL_PATH) + File.separator + tableInfo.getServiceImplName() + suffixJavaOrKt()), entityName);
                    writer(objectMap, templateFilePath(template.getServiceImpl()), implFile, isCreate(FileType.SERVICE_IMPL, implFile));
                }
                // MpController.java
                if (null != tableInfo.getControllerName() && null != pathInfo.get(ConstVal.CONTROLLER_PATH)) {
                    String controllerFile = String.format((pathInfo.get(ConstVal.CONTROLLER_PATH) + File.separator + tableInfo.getControllerName() + suffixJavaOrKt()), entityName);
                    writer(objectMap, templateFilePath(template.getController()), controllerFile, isCreate(FileType.CONTROLLER, controllerFile));
                }
            }
        } catch (Exception e) {
     ...
     */
    public abstract void writer(Map<String, Object> objectMap, String templatePath, String outputFile, boolean isCreate) throws Exception;
